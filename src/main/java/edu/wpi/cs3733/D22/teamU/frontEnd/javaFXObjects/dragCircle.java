        ...
        new EventHandler<MouseEvent>() {
          ...
          public void handle(MouseEvent mouseEvent) {
            ...
            if (getBoundsInParent().intersects(pane.getBoundsInParent())) {
              ...
              if (lnOld.getLocation().getEquipment().size() == 0) lnOld.getChildren().remove(1);
              ...
              copy.setLocation(best.getLocation()); // init the location for the equipmennt
              mc.equipTable.getItems().add(copy);
              mc.equipTable.getItems().remove(equipment);
              */
